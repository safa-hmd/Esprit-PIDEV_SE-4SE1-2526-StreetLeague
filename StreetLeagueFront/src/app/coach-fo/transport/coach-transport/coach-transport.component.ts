...
export class CoachTransportComponent implements OnInit {
  ...
  loadTournaments() {
    this.travelService.getUpcomingTournaments().subscribe({
      next: (res: any[]) => {
  ...
  onTournamentChange() {
    ...
    if (this.selectedTournament && this.teamCity && this.selectedTournament.city) {
  ...
  applyFilter() {
    if (!this.selectedTournament || !this.selectedTournament.city) {
        this.filteredTransports = [];
        return;
    }
    const dest = this.selectedTournament.city.toLowerCase();
    const names = ['John Smith', 'Sarah Johnson', 'Mike Davis', 'Emily Wilson', 'Emma Brown'];
    
    this.filteredTransports = this.transports
      .filter(t => t.destination && t.destination.toLowerCase() === dest)
      .map((t, index) => ({
        ...t,
        ownerName: t.type === 'PRIVATE_CAR' ? names[index % names.length] : 'Official Fleet',
