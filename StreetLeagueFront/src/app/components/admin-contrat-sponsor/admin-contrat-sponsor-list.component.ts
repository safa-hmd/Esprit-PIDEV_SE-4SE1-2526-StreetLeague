...
@Component({
  selector: 'app-admin-contrat-sponsor-list',
  templateUrl: './admin-contrat-sponsor-list.component.html',
  styleUrls: ['./admin-contrat-sponsor-list.component.css']
})
export class AdminContratSponsorListComponent implements OnInit {
  ...
  createNew(): void { this.router.navigate(['/admin/contrat-sponsor/new']); }
  edit(id: number): void { this.router.navigate(['/admin/contrat-sponsor', id, 'edit']); }
  ...
  delete(id: number): void {
    ...
    this.contratService.delete(id).subscribe({
      next: () => {
        ...
        this.successMessage = 'Contrat supprimé.';
        this.loading = false;
