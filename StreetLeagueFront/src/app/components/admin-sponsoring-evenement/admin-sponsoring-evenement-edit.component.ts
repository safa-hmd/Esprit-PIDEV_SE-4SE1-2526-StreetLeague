...
@Component({
  selector: 'app-admin-sponsoring-evenement-edit',
  templateUrl: './admin-sponsoring-evenement-edit.component.html',
  styleUrls: ['./admin-sponsoring-evenement-edit.component.css']
})
export class AdminSponsoringEvenementEditComponent implements OnInit {
  formData = {
    sponsorId: 0,
    evenementId: 0,
    contribution: 0,
  ...
  save(): void {
    ...
    this.sponsoringService.update(this.sponsoringId, payload).subscribe({
      next: () => {
        ...
        this.saving = false;
        setTimeout(() => this.router.navigate(['/admin/sponsoring-evenement']), 1500);
      },
      error: (err) => {
