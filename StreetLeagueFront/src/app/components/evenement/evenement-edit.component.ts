...
export class EvenementEditComponent implements OnInit {
  ...
  loadEvenement(): void {
    ...
    this.evenementService.getById(this.evenementId).subscribe({
      ...
      error: (err) => {
        this.error = 'Impossible de charger l\'événement : ' + err.message;
        this.loading = false;
      }
    });
  }
  ...
  save(): void {
    ...
    if (!this.formData.communauteId || this.formData.communauteId === 0) {
      this.error = 'Veuillez sélectionner une communauté.';
      return;
    }
    ...
    this.evenementService.update(this.evenementId, payload).subscribe({
      ...
      error: (err) => {
        const backendMsg =
          ...
          err.message ||
          'Erreur inconnue';
        this.error = 'Erreur lors de la mise à jour : ' + backendMsg;
        this.saving = false;
      }
    });
  }
