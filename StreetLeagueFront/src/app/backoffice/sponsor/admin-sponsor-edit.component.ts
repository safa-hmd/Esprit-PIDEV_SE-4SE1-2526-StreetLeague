...
@Component({
  ...
  styleUrls: ['./admin-sponsor-edit.component.css']
})
export class AdminSponsorEditComponent implements OnInit {
  formData = { nom: '', type: 'Bronze', contactEmail: '', telephone: '', address: '' };
  ...
  loadData(): void {
    ...
    this.sponsorService.getById(this.sponsorId).subscribe({
      next: (data) => {
        this.formData = { nom: data.nom, type: data.type, contactEmail: data.contactEmail, telephone: data.telephone, address: data.address };
        this.loading = false;
      },
  ...
  save(): void {
    this.error = '';
    if (!this.formData.nom.trim()) { this.error = 'Le name est obligatoire.'; return; }
    ...
    this.sponsorService.update(this.sponsorId, payload).subscribe({
      next: () => { this.successMessage = 'Sponsor mis à jour !'; this.saving = false; setTimeout(() => this.router.navigate(['/admin/sponsor']), 1500); },
