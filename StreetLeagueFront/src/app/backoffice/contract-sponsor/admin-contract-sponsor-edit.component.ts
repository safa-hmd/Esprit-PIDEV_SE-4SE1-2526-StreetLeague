...
@Component({
  selector: 'app-admin-contract-sponsor-edit',
  templateUrl: './admin-contract-sponsor-edit.component.html',
  styleUrls: ['./admin-contract-sponsor-edit.component.css']
})
export class AdminContractSponsorEditComponent implements OnInit {
  formData = {
    sponsorId: 0,
    equipeId: 0,
    montantTotal: 0,
  ...
  save(): void {
    ...
    this.contractService.update(this.contractId, payload).subscribe({
      next: () => {
        this.successMessage = 'Contract mis à jour !';
        this.saving = false;
        setTimeout(() => this.router.navigate(['/admin/contract-sponsor']), 1500);
      },
      error: (err) => {
