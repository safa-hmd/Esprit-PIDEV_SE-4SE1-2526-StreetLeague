...
export class AdminEvenementEditComponent implements OnInit {
  ...
  save(): void {
    this.error = '';
    if (!this.formData.titre.trim()) { this.error = 'Le title est obligatoire.'; return; }
    ...
    if (!this.formData.communauteId || this.formData.communauteId <= 0) { this.error = 'Please select une community.'; return; }
    ...
    this.evenementService.update(this.evenementId, payload).subscribe({
      next: () => {
        this.successMessage = 'Event mis à jour !';
        this.saving = false;
