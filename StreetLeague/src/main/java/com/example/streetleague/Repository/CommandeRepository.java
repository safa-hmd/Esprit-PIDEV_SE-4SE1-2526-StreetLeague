...
public interface CommandeRepository extends JpaRepository<Commande, Long> {
    @Query("SELECT COUNT(c) FROM Commande c WHERE c.user.idUser = :userId AND c.statut = :statut")
    long countByUserIdAndStatut(@Param("userId") Long userId, @Param("statut") CommandeStatus statut);
