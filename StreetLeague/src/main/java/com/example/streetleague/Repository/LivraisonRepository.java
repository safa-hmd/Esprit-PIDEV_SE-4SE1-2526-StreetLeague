...
public interface LivraisonRepository extends JpaRepository<Livraison, Long> {
    ...
    List<Livraison> findByLivreur_IdUser(Long livreurId);

    // Stats
    long countByStatut(LivraisonStatus statut);

    List<Livraison> findByLivreur_IdUserAndStatutIn(Long livreurId, List<LivraisonStatus> statuts);
    List<Livraison> findByDateCreationAfter(LocalDateTime debut);

    @Query("""
    SELECT l.livreur.idUser,
           l.livreur.fullName,
           COUNT(l),
    ...
    GROUP BY l.livreur.idUser, l.livreur.fullName
